...
public sealed class WindowsCompanionRuntime : IPrimaryAppRuntime, ICompanionEventSink
{
    ...
    private async Task<bool> StartCoreAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                // Logging only: hotkey/tray attach failures (conflict,
                // unavailable) still propagate to the bootstrap failure path,
                // now with an operation name plus the exception type. The
                // services above already self-report more specific
                // operations; this captures the composed attach step.
                ReportFailure("startup-chrome-attach", exception);
                throw;
            }
