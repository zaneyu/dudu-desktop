...
public sealed class PreferencesRepository : SqliteRepository, IPreferencesRepository
{
    ...
    {
        await using var connection = await OpenAsync(cancellationToken); await using var command = connection.CreateCommand(); command.CommandText = "SELECT theme,quiet_hours_enabled,quiet_hours_start,quiet_hours_end,reduced_motion,local_note_daily_limit,launch_at_sign_in,always_on_top,hide_pet_during_fullscreen,ambient_minimum_interval_ticks,hydration_reminders_enabled,break_reminders_enabled,outfit_key,automatic_seasonal_mode,anniversary_month,anniversary_day,birthday_month,birthday_day,evening_check_in_enabled,bedtime_ritual_enabled,sounds_enabled,sound_volume FROM preferences WHERE id=1;";
