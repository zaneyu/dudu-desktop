...
public sealed class FocusSessionRepository : SqliteRepository, IFocusSessionRepository
{
    ...
    public async Task<bool> TryCreateActiveAsync(FocusSession session, CancellationToken cancellationToken)
    {
        if (IsTransactionBound)
        {
            ...
            boundCommand.CommandText = """
                INSERT INTO focus_sessions (id, task_id, started_utc, ends_utc, remaining_when_paused_ticks, consumed_focus_ticks, status, updated_utc)
                SELECT $id, $task, $started, $ends, $remaining, $consumed, $status, $updated
        ...
        try
        {
            ...
            command.CommandText = """
                UPDATE focus_sessions SET task_id=$newTask, started_utc=$newStarted, ends_utc=$newEnds,
                    remaining_when_paused_ticks=$newRemaining, consumed_focus_ticks=$newConsumed, status=$newStatus, updated_utc=$newUpdated
                WHERE id=$id AND task_id IS $oldTask AND started_utc=$oldStarted AND ends_utc IS $oldEnds
                    AND remaining_when_paused_ticks=$oldRemaining AND consumed_focus_ticks=$oldConsumed AND status=$oldStatus AND updated_utc=$oldUpdated;
                """;
            ...
            Add(command, "$oldConsumed", expected.ConsumedFocusTime.Ticks);
            ...
            Add(command, "$newConsumed", replacement.ConsumedFocusTime.Ticks);
    ...
    public async Task SaveAsync(FocusSession session, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            INSERT INTO focus_sessions (id, task_id, started_utc, ends_utc, remaining_when_paused_ticks, consumed_focus_ticks, status, updated_utc)
            VALUES ($id,$task,$started,$ends,$remaining,$consumed,$status,$updated)
            ON CONFLICT(id) DO UPDATE SET task_id=excluded.task_id, started_utc=excluded.started_utc, ends_utc=excluded.ends_utc,
                remaining_when_paused_ticks=excluded.remaining_when_paused_ticks, consumed_focus_ticks=excluded.consumed_focus_ticks, status=excluded.status, updated_utc=excluded.updated_utc;
            """;
        AddSession(command, session); await command.ExecuteNonQueryAsync(cancellationToken);
    }

    private const string Select = "SELECT id, task_id, started_utc, ends_utc, remaining_when_paused_ticks, consumed_focus_ticks, status, updated_utc FROM focus_sessions";
    ...
    { Add(c,"$id",s.Id.ToString("D")); Add(c,"$task",s.TaskId?.ToString("D")); Add(c,"$started",Utc(s.StartedUtc)); Add(c,"$ends",Utc(s.EndsUtc)); Add(c,"$remaining",s.RemainingWhenPaused.Ticks); Add(c,"$consumed",s.ConsumedFocusTime.Ticks); Add(c,"$status",(int)s.Status); Add(c,"$updated",Utc(s.UpdatedUtc)); }
    private static FocusSession Read(SqliteDataReader r) => new(Guid.Parse(r.GetString(0)), r.IsDBNull(1) ? null : Guid.Parse(r.GetString(1)), ReadUtc(r[2]), ReadNullableUtc(r[3]), TimeSpan.FromTicks(r.GetInt64(4)), (FocusStatus)r.GetInt32(6), ReadUtc(r[7]), TimeSpan.FromTicks(r.GetInt64(5)));
}
